...
export default function Umkm({ umkm, filters = {} }) {
    ...
    useEffect(() => {
        const isChanged = search !== (filters.search || '') || 
                          sektorFilter !== (filters.sektor || '') || 
                          statusFilter !== (filters.status || '');

        if (isChanged) {
            const timeout = setTimeout(() => {
                router.get(route('rw.umkm'), { search, sektor: sektorFilter, status: statusFilter }, { preserveState: true });
            }, 400);
    ...
    return (
        <RwLayout header="Data UMKM">
            <Head title="Data UMKM - RW" />
                ...
                <div className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Data UMKM Wilayah RW</h2>
                                <p className="text-sm text-slate-500 dark:text-slate-400">Monitoring usaha mikro, kecil, dan menengah di seluruh RT.</p>
                            </div>
                        </div>
                        <div className="flex flex-wrap gap-3">
                            <div className="relative">
                                ...
                                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                                    className="block w-full rounded-lg border-0 py-2.5 pl-10 pr-3 text-slate-900 dark:text-white ring-1 ring-inset ring-slate-300 dark:ring-slate-700 placeholder:text-slate-400 focus:ring-2 focus:ring-teal-600 sm:text-sm bg-slate-50 dark:bg-slate-800/50"
                            ...
                            {umkm.data.length > 0 ? umkm.data.map((u) => (
                                <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-600 dark:text-slate-400">
                                        {u.resident?.family_card?.wilayah ? `RT ${u.resident.family_card.wilayah.rt} / RW ${u.resident.family_card.wilayah.rw}` : '-'}
                                    </td>
                                    <td className="whitespace-nowrap px-3 py-4">
                                ...
                                <tr><td colSpan="7" className="px-6 py-12 text-center text-sm text-slate-500 dark:text-slate-400">Belum ada data UMKM terdaftar.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                        ...
                        <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm">
                            {umkm.links.map((link, i) => (
                                <Link 
                                    key={i} 
                                    href={link.url || '#'} 
                                    className={`relative inline-flex items-center px-4 py-2 text-sm font-semibold ${link.active ? 'z-10 bg-teal-600 text-white' : 'text-slate-900 dark:text-white ring-1 ring-inset ring-slate-300 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'} ${i === 0 ? 'rounded-l-md' : ''} ${i === umkm.links.length - 1 ? 'rounded-r-md' : ''}`} 
