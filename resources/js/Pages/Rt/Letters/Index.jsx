...
export default function Index({ letters, filters = {} }) {
    return (
        <RtLayout header="Persetujuan Surat">
            <Head title="Surat - RT" />
            ...
            <div className="mb-6">
                <div className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Surat Masuk dari Warga</h2>
                            <p className="text-sm text-slate-500 dark:text-slate-400">Review dan setujui pengajuan surat warga di RT Anda.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead className="bg-slate-50 dark:bg-slate-800/50">
                            <tr>
                                <th className="py-3.5 pl-6 pr-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Pemohon</th>
                                <th className="px-3 py-3.5 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Jenis Surat</th>
                                <th className="px-3 py-3.5 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Keperluan</th>
                                <th className="px-3 py-3.5 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Tanggal</th>
                                ...
                                <th className="py-3.5 pl-3 pr-6"><span className="sr-only">Aksi</span></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                            {letters.data.length > 0 ? letters.data.map((l) => (
                                <tr key={l.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                    <td className="whitespace-nowrap py-4 pl-6 pr-3">
                                        <div className="flex items-center">
                                            <div className="h-10 w-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-400 font-bold mr-3">
                                                {l.resident?.nama_lengkap?.charAt(0) || '?'}
                                            </div>
                                            <div>
                                                <div className="font-bold text-slate-900 dark:text-white text-sm">{l.resident?.nama_lengkap}</div>
                                                <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">{l.resident?.nik}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-500 dark:text-slate-400">{new Date(l.created_at).toLocaleDateString('id-ID')}</td>
                                    ...
                                    <td className="whitespace-nowrap py-4 pl-3 pr-6 text-right">
                                        {l.status === 'diajukan' ? (
                                            ...
                                            <Link href={route('rt.letters.show', l.id)} className="text-sm font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-500">Detail</Link>
                                        )}
                                    </td>
                                </tr>
                            )) : (
                                <tr>
                                    <td colSpan="6" className="px-6 py-16 text-center">
                                        ...
                                        <h3 className="text-base font-bold text-slate-900 dark:text-white mb-1">Tidak Ada Surat</h3>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">Belum ada pengajuan surat dari warga.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-6 py-4 flex items-center justify-center">
                        <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm">
                            {letters.links.map((link, i) => (
                                <a key={i} href={link.url || '#'} className={`relative inline-flex items-center px-4 py-2 text-sm font-semibold ${link.active ? 'z-10 bg-blue-600 text-white' : 'text-slate-900 dark:text-white ring-1 ring-inset ring-slate-300 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'} ${i === 0 ? 'rounded-l-md' : ''} ${i === letters.links.length - 1 ? 'rounded-r-md' : ''}`} dangerouslySetInnerHTML={{ __html: link.label }} />
                            ))}
                        </nav>
                    </div>
                )}
